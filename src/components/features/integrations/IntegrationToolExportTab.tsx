...
interface IntegrationToolExportTabProps {
...
export function IntegrationToolExportTab({
  integrationId,
  integrationName,
  integrationSlug,
}: IntegrationToolExportTabProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold">Export as AI Tools</h2>
          <p className="text-sm text-muted-foreground">
            Export actions as AI tool definitions for LLMs and agent frameworks
          </p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          ...
          <CardContent>
            <p className="text-sm">Works with OpenAI, Anthropic, Gemini, and LangChain</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <p className="text-sm">Includes TypeScript and Python code snippets</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
          ...
          <CardContent>
            <p className="text-sm">Ready-to-use MCP server for Claude Desktop</p>
          </CardContent>
        </Card>
      </div>
        ...
        <TabsContent value="universal" className="space-y-4">
          {universalLoading ? (
            ...
            <div className="space-y-4">
                ...
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <CardDescription>
                        LLM-agnostic format compatible with all major providers
                      </CardDescription>
                    </div>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-sm">Compatible With</CardTitle>
                </CardHeader>
                <CardContent>
              ...
              {universalData.contextTypes.length > 0 && (
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Reference data types that can be injected at invocation time
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="flex flex-wrap gap-2">
                ...
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <CardDescription>JSON schemas for all tools</CardDescription>
                    </div>
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() =>
                        handleDownload(
                          `${integrationSlug}-langchain-schemas.json`,
                          JSON.stringify(langchainData.tools, null, 2)
                        )
                      }
                    >
                      <Download className="mr-2 h-4 w-4" />
                ...
                <CardContent>
                  <div className="relative">
                    <pre className="max-h-64 overflow-auto rounded-lg bg-muted p-4 text-xs">
                      <code>{JSON.stringify(langchainData.tools, null, 2)}</code>
                    </pre>
                    <CopyButton
                      value={JSON.stringify(langchainData.tools, null, 2)}
                      label="Copied to clipboard"
                      className="absolute right-2 top-2"
                    />
                  </div>
            ...
            <div className="space-y-4">
              ...
              <Card>
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <CardDescription>TypeScript MCP server implementation</CardDescription>
                    </div>
                    <Button
                      variant="outline"
                      size="sm"
                ...
                <CardContent>
                  <div className="relative">
                    <pre className="max-h-80 overflow-auto rounded-lg bg-muted p-4 text-xs">
                      <code>{mcpData.serverFile.typescript}</code>
                    </pre>
                    <CopyButton
                      value={mcpData.serverFile.typescript}
                      label="Copied to clipboard"
                      className="absolute right-2 top-2"
                    />
                  </div>
              ...
              <Card>
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div>
                      <CardTitle>Server Definition</CardTitle>
                      <CardDescription>MCP server capabilities and tools</CardDescription>
                    </div>
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() =>
                        handleDownload(
                          `${integrationSlug}-mcp-definition.json`,
                          JSON.stringify(mcpData.server, null, 2)
                        )
                      }
                    >
                      <Download className="mr-2 h-4 w-4" />
                ...
                <CardContent>
                  <div className="relative">
                    <pre className="max-h-64 overflow-auto rounded-lg bg-muted p-4 text-xs">
                      <code>{JSON.stringify(mcpData.server, null, 2)}</code>
                    </pre>
                    <CopyButton
                      value={JSON.stringify(mcpData.server, null, 2)}
                      label="Copied to clipboard"
                      className="absolute right-2 top-2"
                    />
                  </div>
              ...
              {/* Resources Info */}
              {mcpData.server.resources.length > 0 && (
                <Card>
                  <CardHeader>
                    <CardTitle className="text-sm">Available Resources</CardTitle>
                    <CardDescription>
                      Reference data exposed as MCP resources for context resolution
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-2">
                      {mcpData.server.resources.map((resource) => (
                        <div
                          key={resource.uri}
