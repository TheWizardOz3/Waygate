/**
 * LangChain Tools Export Endpoint
 *
 * GET /api/v1/integrations/:id/tools/langchain - Export all actions as LangChain-compatible tools
 */

import { NextRequest, NextResponse } from 'next/server';
import { withApiAuth } from '@/lib/api/middleware/auth';
import { exportIntegrationToolsLangChain, ToolExportError } from '@/lib/modules/tool-export';
...
export const GET = withApiAuth(async (request: NextRequest, { tenant }) => {
  try {
    ...
    const options = {
      ...
      includeCodeSnippets: searchParams.get('includeCodeSnippets') !== 'false',
      apiBaseUrl: searchParams.get('apiBaseUrl') || undefined,
    };

    // Export tools
    const result = await exportIntegrationToolsLangChain(tenant.id, integrationId, options);
    ...
    console.error('[ToolExport] LangChain export error:', error);
    return NextResponse.json(
      {
        success: false,
        error: {
