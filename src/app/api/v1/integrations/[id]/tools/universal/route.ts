/**
 * Universal Tools Export Endpoint
 *
 * GET /api/v1/integrations/:id/tools/universal - Export all actions as LLM-agnostic tools
 */

import { NextRequest, NextResponse } from 'next/server';
import { withApiAuth } from '@/lib/api/middleware/auth';
import { exportIntegrationToolsUniversal, ToolExportError } from '@/lib/modules/tool-export';
...
export const GET = withApiAuth(async (request: NextRequest, { tenant }) => {
  try {
    ...
    // Export tools
    const result = await exportIntegrationToolsUniversal(tenant.id, integrationId, options);
    ...
    console.error('[ToolExport] Universal export error:', error);
    return NextResponse.json(
      {
        success: false,
        error: {
